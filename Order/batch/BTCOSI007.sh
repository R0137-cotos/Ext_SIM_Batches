...
### 処理実行
################################################
SPRING_PROFILES_ACTIVE=${ENVIRONMENT_NAME} /usr/bin/java -Dlogging.file.name=${PROCESS_LOG_FILE_PATH} -jar ${ORDER_JAR_PATH}/${BATCH_PG_BTCOSI007} "${OPERATION_DATE}" "${DIR_PATH}" "${FILE_NAME}" > /dev/null 2>&1
BATCH_RET=$?
if [  ${BATCH_RET} == 1 ]; then
