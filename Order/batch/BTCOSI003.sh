...
### 処理実行
################################################
SPRING_PROFILES_ACTIVE=${ENVIRONMENT_NAME} /usr/bin/java -Dlogging.file.name=${PROCESS_LOG_FILE_PATH} -jar ${ORDER_JAR_PATH}/${BATCH_PG_BTCOSI003} "${DIR_PATH}" "${FILE_NAME}" > /dev/null 2>&1
BATCH_RET=$?
if [  ${BATCH_RET} == 2 ]; then
  Log.Error "BTCOSI003:[SB]リプライCSV取込が一部失敗しました。ログを確認してください。" >> ${LOG_FILE_PATH};
  exit 2
