...
### 処理実行
################################################
SPRING_PROFILES_ACTIVE=${ENVIRONMENT_NAME} /usr/bin/java -Dlogging.file=${PROCESS_LOG_FILE_PATH} -jar ${COMMUNICATION_JAR_PATH}/${BATCH_PG_BTCOSI002} "${DIR_PATH}" "${FILE_NAME}" "${PRODUCT_GRP_MASTER_ID}" "${MAIL_ADDRESS_LIST}" > /dev/null 2>&1
BATCH_RET=$?
if [  ${BATCH_RET} != 0 ]; then
