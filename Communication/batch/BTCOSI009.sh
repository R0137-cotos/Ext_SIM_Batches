...
Log.Info "サービス開始日：${DIR_PATH}" >> ${LOG_FILE_PATH}

################################################
### 処理実行
################################################
SPRING_PROFILES_ACTIVE=${ENVIRONMENT_NAME} /usr/bin/java -jar ${COMMUNICATION_JAR_PATH}/${BATCH_PG_BTCOSI009} "${SERVICE_TERM_START}" > ${PROCESS_LOG_FILE_PATH}
BATCH_RET=$?
if [  ${BATCH_RET} != 0 ]; then
