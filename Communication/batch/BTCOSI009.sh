...
### 処理実行
################################################
SPRING_PROFILES_ACTIVE=${ENVIRONMENT_NAME} /usr/bin/java -Dlogging.file.name=${PROCESS_LOG_FILE_PATH} -jar ${COMMUNICATION_JAR_PATH}/${BATCH_PG_BTCOSI009} "${SERVICE_TERM_START}" > /dev/null 2>&1
BATCH_RET=$?
if [  ${BATCH_RET} == 2 ]; then
