...
public class AccountingExecution {
	...
	private void saveAccounting() {
		...
		// 27 NSPユニークキー
		// 契約.契約番号(頭4桁除去)＋品種（契約用）.リコー品種コード＋計上ID
		this.accounting.setFfmNspKey(this.accounting.getFfmContractDocNo().substring(4) + this.accounting.getProductTypeCd() + this.accounting.getId());
		accountingRepository.save(this.accounting);
	}
